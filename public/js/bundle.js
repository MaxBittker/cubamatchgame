...
/***/ function(module, exports, __webpack_require__) {
	...
	var acDiv = document.getElementById("ActorCards");
	var spDiv = document.getElementById("spoilers");

	var spinner = document.getElementById("loading");
	var smallspinner = document.getElementById("loadingSecondary");
	smallspinner.style.visibility = "hidden";
	var moviesearch = document.getElementById("movie");
	var go = document.getElementById("go");
	var optionsdiv = document.getElementById("options");
	var titleEl = document.getElementById("title");
	var prompt = document.getElementById("namePrompt");
	...
	function getMovies(query) {
	  ...
	  request("/api/queryMovie/" + query, function (err, res, bod) {
	    if (err) throw err;
	    bod = JSON.parse(bod);
	...
	function getActors(id) {
	  ...
	  request("/api/getActors/" + id, function (err, res, bod) {
	    if (err) throw err;
	    bod = JSON.parse(bod);
	    spDiv.className = "";
	...
	function actorCards(actors) {
	  var pageCards = _.map(actors, buildActorCard);
	  acDiv.innerHTML = "";
	  pageCards.forEach(function (card) {
	    return acDiv.appendChild(card);
	  });
	  var spoilers = _.map(actors, buildSpoiler);
	  spDiv.innerHTML = "";
	  spoilers.forEach(function (spoiler) {
	    return spDiv.appendChild(spoiler);
	  });
	}
	...
	var buildActorCard = function buildActorCard(actor) {
	...
	var buildMovieOptions = function buildMovieOptions(options) {
	  var optionList = document.createElement("ul");
	  options.forEach(function (movie) {
	    var el = document.createElement("li");
	    var label = document.createTextNode(movie.title);
	...
	go.addEventListener("click", function () {
	  return getMovies(moviesearch.value);
	});
	if (window.location.hash) getActors(window.location.hash.slice(1));else getActors("0120737");
